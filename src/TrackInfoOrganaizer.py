...
global_BPM_detection_threashold = 1800
global_BPM_detection_windowsize = 2
global_multithread_maxWorkers = 8
global_encoding = 'utf-8-sig'

class ProcessFile(threading.Thread):
    def __init__(self, filename, filepath, exiting_files, csv_export, lock):
        threading.Thread.__init__(self)
        self.filename = filename
        self.filepath = filepath
        self.exiting_files = exiting_files
        self.csv_export = csv_export
        self.lock = lock

    def run(self):
        self.record_file_info(self.filename, self.filepath, self.exiting_files, self.csv_export, self.lock)
    ...
    def bpm_detector(self, data, fs):
        ...
        if len(peak_ndx) > 1:
            return self.no_audio_data(self)
    ...
    def process_file(self, filepath, filename):
        samps, fs = self.read_wav(str(filepath))
    ...
    def record_file_info(self, filename, filepath, exiting_files, csv_export, lock):
        csv_entry = {}
        bpm = None
        key = ''
                ...
                with taglib.File(filepath, save_on_exit=True) as track:
                    ...
                    if bpm == '0' or bpm == '':
                        if 'BPM' in track.tags:
                            ...
                            bpm = self.process_file(filepath, filename)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--window",
        type=float,
        default=global_BPM_detection_windowsize,
        ...
        if dirpath == folder_path:
            for filename in filenames:
                ...
                if file_extension == '.wav':
                    ...
                    threads.append(ProcessFile(filename, filepath, exiting_files, csv_export, lock))
                    itemCount = itemCount + 1

    with ThreadPoolExecutor(max_workers = global_multithread_maxWorkers) as executor:
        for thread in threads:
            executor.submit(StartThread, thread)
