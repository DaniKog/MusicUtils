...
def record_file_info(filename, filepath, exiting_files, csv_export):
    csv_entry = {}
    bpm = None
    key = ''
        ...
        try:
            with taglib.File(filepath, save_on_exit=True) as track:
                ...
                if 'COMMENT' in track.tags:
                    ...
                    hasChanges = True
                ## update BPM from csv
                if bpm == '0' or bpm == '' or bpm == '-1':
                    if 'BPM' in track.tags and bpm != '-1': ## -1 means re-process
                        ...
                        logs.append(f'BPM was missing in csv {filename} updated to {bpm}')
                    hasChanges = True
